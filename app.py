    ...
    if uploaded_file is not None:
        ...
        if valid_url(website_url):
            st.markdown(
                ...
                <div class="empty-state">
                    ...
                    <div class="empty-title">Website URL captured</div>
                    <p class="empty-desc">The address is ready for analysis.<br>Press Start UX Audit to process.</p>
                </div>
                """,
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                ...
                <div class="empty-state">
                    <div class="empty-icon">!</div>
                    <div class="empty-title">Enter a valid URL</div>
                    <p class="empty-desc">Include https:// or http://, for example:<br>https://nova-atelier.com</p>
                </div>
                """,
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            ...
            <div class="empty-state">
                ...
                <p class="empty-desc">
                    Upload a screenshot or wireframe, or switch to <strong>Website URL</strong>
                    to audit a live page.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
...
with results_col:
    ...
    # Donut score (placeholder — no fake results)
    st.markdown(
        """
        <div class="score-donut">
            <div class="donut-ring">
                <svg width="72" height="72" viewBox="0 0 72 72">
                    <circle cx="36" cy="36" r="30" fill="none" stroke="#142238" stroke-width="8"/>
                    <circle cx="36" cy="36" r="30" fill="none" stroke="#3B82F6" stroke-width="8"
                        stroke-dasharray="141.4" stroke-dashoffset="141.4" stroke-linecap="round"/>
                </svg>
                <div class="donut-text">
                    <div class="donut-value">—</div>
                    <div class="donut-grade">—</div>
                </div>
            </div>
            <div>
                <div class="score-info-title">Waiting for audit</div>
                <div class="score-info-desc">
                    Upload a source and start the scan to generate the overall score.
                </div>
            </div>
        </div>
        """,
    ...
    # Metrics (all placeholder)
    metrics = [
        ("Visual Hierarchy", "—%", "—", "#3B82F6"),
        ("Usability", "—%", "—", "#22C55E"),
    ...
    c1, c2 = st.columns(2)
    for i, (label, val, grade, color) in enumerate(metrics):
            ...
            st.markdown(
                ...
                <div class="metric-box">
                    ...
                    <div class="metric-value">
                        <span class="metric-number" style="color:#4A6388;">{val}</span>
                        <span class="metric-grade" style="background:rgba(100,100,100,0.15);color:#5A7399;">
                            {grade}
                        </span>
                    </div>
                </div>
    ...
    st.markdown(
        ...
        <div class="insights-header">
            <span class="insights-title">Actionable Insights</span>
            <span class="insights-count">0 items</span>
        </div>
    ...
    # Start button
    can_start = uploaded_file is not None or (website_url.strip() and valid_url(website_url))

    if st.button("Start UX Audit", type="primary", use_container_width=True, disabled=not can_start):
        st.session_state.audit_requested = True
        st.session_state.audit_name = (
            audit_name.strip()
            or (uploaded_file.name if uploaded_file else urlparse(website_url).netloc)
        )
        st.session_state.audit_source = (
            uploaded_file.name if uploaded_file else website_url.strip()
        )

    if not can_start:
        st.caption("Add a screenshot or valid URL to continue.")

    st.markdown("<​/div>", unsafe_allow_html=True)

# ═════════════════════════════════════════════
# Audit requested confirmation
# ═════════════════════════════════════════════
if st.session_state.audit_requested:
    st.success(
        f"Audit request registered: **{st.session_state.audit_name}**. "
        "Connect an analysis engine to generate real UX findings."
    )
    with st.expander("Audit request details"):
